define(function (require) {
  ...
  require(["domReady!"], function (doc) {
    ...
    function saveState() {
      ...
      undoStack.push(drawingCanvas.graphics.clone());

      // Clear the redo stack when new actions are made
      redoStack = [];
    }

    function undo() {
      if (undoStack.length > 1) {
        // Remove the current state from undo stack and push it to redo stack
        redoStack.push(undoStack.pop());

        // Restore the previous state from the undo stack
        drawingCanvas.graphics = undoStack[undoStack.length - 1].clone();
    ...
    function handleImageLoad(event) {
      ...
      bitText = new createjs.Text(
        image.dataId.toString(),
        "bold 20px Arial",
        "#000"
      );
